...
export function PopupDuplicateCheck({ locale, summary, checkedSummary, loading, matches, onCheck }: {
  ...
  return (
    <div className={`mt-2 rounded-[var(--qm-control-radius)] border p-2.5 ${matches.length ? "border-warning/30 bg-warning/5" : "border-dashed bg-muted/10"}`}>
      ...
      {matches.length ? <div className="mt-2 grid gap-1.5">{matches.map(({ issue, score }) => <button key={issue.key} type="button" onClick={() => window.open(jiraBrowseUrl(issue.key), "_blank")} className="flex min-w-0 items-center gap-2 rounded-[var(--qm-control-radius)] border bg-background px-2 py-1.5 text-start"><span className="shrink-0 font-mono text-[10px] text-muted-foreground">{issue.key}</span><span className="min-w-0 flex-1 truncate text-[11px]">{issue.summary}</span><span className="shrink-0 text-[10px] font-semibold text-warning">{score}%</span></button>)}</div> : null}
    </div>
  )
}
