...
export function PopupEvidenceSection({ t, context, screenshotCount, attachments, diagnostics, includeContext, includeScreenshot, includeDiagnostics, capturing, onAttachments, onIncludeContext, onIncludeScreenshot, onIncludeDiagnostics, onAddCapture }: {
  ...
  return (
    <div className="qm-field-span-2 rounded-xl border bg-card p-3">
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-3">
        {context ? <label className="flex items-center gap-2 rounded-lg border bg-background/45 px-3 py-2 text-xs font-medium"><MonitorUp className="size-3.5 text-primary" /><input type="checkbox" checked={includeContext} onChange={(event) => onIncludeContext(event.target.checked)} className="accent-[var(--primary)]" />{t.includeContext}</label> : null}
        {screenshotCount ? <label className="flex items-center gap-2 rounded-lg border bg-background/45 px-3 py-2 text-xs font-medium"><Image className="size-3.5 text-primary" /><input type="checkbox" checked={includeScreenshot} onChange={(event) => onIncludeScreenshot(event.target.checked)} className="accent-[var(--primary)]" />{t.includeScreenshot} · {screenshotCount}</label> : null}
        {diagnostics ? <label className="flex items-center gap-2 rounded-lg border bg-background/45 px-3 py-2 text-xs font-medium"><Activity className="size-3.5 text-primary" /><input type="checkbox" checked={includeDiagnostics} onChange={(event) => onIncludeDiagnostics(event.target.checked)} className="accent-[var(--primary)]" />{t.includeDiagnostics}</label> : null}
      </div>
      ...
      {diagnostics ? <div className="mt-2 rounded-lg bg-muted/35 px-3 py-2 text-[11px] text-muted-foreground"><div className="font-medium text-foreground">{t.diagnostics}</div><div className="mt-0.5">{stats.errors} errors · {stats.failedRequests} failed requests · {stats.requests} recent network entries</div><div className="mt-1 leading-4">{t.diagnosticsHint}</div></div> : null}
