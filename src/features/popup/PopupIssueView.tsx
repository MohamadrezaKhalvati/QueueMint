...
export type PopupIssueViewProps = {
  ...
  issueType: string; createFieldIds: string[] | null; assignees: JiraUser[]; assignee: string; priority: string; epics: JiraEpic[]; epic: string; description: string; moreFields: boolean; estimate: string; storyPoints: string; labels: string; dueDate: string; component: string; fixVersion: string
    ...
    <div className="space-y-3">
      ...
      {loadingMetadata ? <div className="grid min-h-64 place-items-center rounded-[var(--qm-panel-radius)] border bg-card"><div className="flex items-center gap-2 text-sm text-muted-foreground"><LoaderCircle className="size-4 animate-spin" />{t.loadingJira}</div></div> : metadata ? <>
        ...
        <div className="qm-capture-issue-form">
          ...
          {moreFields ? <PopupIssueExtraFields t={t} locale={locale} metadata={metadata} projectInfo={projectInfo} issueType={issueType} createFieldIds={createFieldIds} estimate={estimate} storyPoints={storyPoints} labels={labels} dueDate={dueDate} component={component} fixVersion={fixVersion} onEstimate={props.onEstimate} onStoryPoints={props.onStoryPoints} onLabels={props.onLabels} onDueDate={props.onDueDate} onComponent={props.onComponent} onFixVersion={props.onFixVersion} /> : null}
