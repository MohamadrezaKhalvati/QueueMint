...
export function SprintSelect({ sprints, value, defaultSprint, onValueChange, allowInherited = true, inheritedLabel = "Use batch default", noDefaultLabel = "No default", backlogLabel = "Backlog / no sprint", activeLabel = "Active", futureLabel = "Future", disabled, className }: {
  sprints: JiraSprint[]; value?: number | null; defaultSprint?: number | null; onValueChange: (value: number | null | undefined) => void; allowInherited?: boolean; inheritedLabel?: string; noDefaultLabel?: string; backlogLabel?: string; activeLabel?: string; futureLabel?: string; disabled?: boolean; className?: string
}) {
  const encoded = allowInherited ? value === undefined ? DEFAULT_VALUE : value === null ? NONE_VALUE : String(value) : value === undefined ? UNSET_VALUE : value === null ? NONE_VALUE : String(value)
  const effectiveId = value !== undefined ? value : defaultSprint
  const selectedSprint = typeof effectiveId === "number" ? sprints.find((item) => item.id === effectiveId) : undefined
  const inherited = allowInherited && value === undefined && typeof defaultSprint === "number"
  return <Select value={encoded} disabled={disabled} onValueChange={(next) => { if (next === DEFAULT_VALUE || next === UNSET_VALUE) onValueChange(undefined); else if (next === NONE_VALUE) onValueChange(null); else onValueChange(Number(next)) }}>
    ...
    <SelectContent>
      {allowInherited ? <SelectItem value={DEFAULT_VALUE}><div className="flex items-center gap-2"><span className="grid size-6 place-items-center rounded-md bg-muted text-muted-foreground">↳</span><div><div className="font-medium">{inheritedLabel}</div><div className="text-[11px] text-muted-foreground">{typeof defaultSprint === "number" ? sprints.find((item) => item.id === defaultSprint)?.name ?? defaultSprint : backlogLabel}</div></div></div></SelectItem> : <SelectItem value={UNSET_VALUE}><span className="text-muted-foreground">{noDefaultLabel}</span></SelectItem>}
      <SelectItem value={NONE_VALUE}><SprintVisual backlogLabel={backlogLabel} compact /></SelectItem><SelectSeparator />
