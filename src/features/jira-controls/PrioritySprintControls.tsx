...
export function PrioritySelect({ priorities, value, defaultPriority, onValueChange, allowInherited = true, noDefaultLabel = "No default", inheritedLabel = "Use batch default", disabled, className }: {
  ...
  return <Select value={encoded} disabled={disabled} onValueChange={(next) => onValueChange(next === DEFAULT_VALUE || next === UNSET_VALUE ? undefined : next)}>
    ...
    <SelectContent>
      {allowInherited ? <><SelectItem value={DEFAULT_VALUE}><div className="flex min-w-0 items-center gap-2"><span className="grid size-6 place-items-center rounded-[var(--qm-control-radius)] bg-muted text-muted-foreground">↳</span><div className="min-w-0"><div className="font-medium">{inheritedLabel}</div>{defaultPriority ? <div className="text-[11px] text-muted-foreground">{defaultPriority}</div> : null}</div></div></SelectItem><SelectSeparator /></> : <><SelectItem value={UNSET_VALUE}><span className="text-muted-foreground">{noDefaultLabel}</span></SelectItem><SelectSeparator /></>}
...
export function SprintVisual({ sprint, inherited = false, inheritedLabel = "default", backlogLabel = "Backlog", compact = false }: {
  ...
  if (!sprint) return <span className="inline-flex min-w-0 items-center gap-2 text-muted-foreground"><span className="grid size-6 shrink-0 place-items-center rounded-[var(--qm-control-radius)] bg-muted"><CalendarClock className="size-3.5" /></span><span className="truncate">{backlogLabel}</span></span>
  const active = sprint.state === "active"
  return <span className="inline-flex min-w-0 items-center gap-2"><span className={cn("grid shrink-0 place-items-center rounded-[var(--qm-control-radius)]", compact ? "size-6" : "size-7", active ? "bg-success/10 text-success" : "bg-primary/10 text-primary")}><CircleDot className={compact ? "size-3.5" : "size-4"} /></span><span className="min-w-0 truncate"><span className="font-medium">{sprint.name}</span>{inherited ? <span className="ms-1 text-[11px] font-normal text-muted-foreground">{inheritedLabel}</span> : null}</span></span>
}
...
export function SprintSelect({ sprints, value, defaultSprint, onValueChange, allowInherited = true, allowBacklog = true, inheritedLabel = "Use batch default", noDefaultLabel = "No default", backlogLabel = "Backlog / no sprint", activeLabel = "Active", futureLabel = "Future", disabled, className }: {
  ...
  return <Select value={encoded} disabled={disabled} onValueChange={(next) => { if (next === DEFAULT_VALUE || next === UNSET_VALUE) onValueChange(undefined); else if (next === NONE_VALUE) onValueChange(null); else onValueChange(Number(next)) }}>
    ...
    <SelectContent>
      {allowInherited ? <SelectItem value={DEFAULT_VALUE}><div className="flex items-center gap-2"><span className="grid size-6 place-items-center rounded-[var(--qm-control-radius)] bg-muted text-muted-foreground">↳</span><div><div className="font-medium">{inheritedLabel}</div><div className="text-[11px] text-muted-foreground">{typeof defaultSprint === "number" ? sprints.find((item) => item.id === defaultSprint)?.name ?? defaultSprint : backlogLabel}</div></div></div></SelectItem> : allowBacklog ? <SelectItem value={UNSET_VALUE}><span className="text-muted-foreground">{noDefaultLabel}</span></SelectItem> : null}
