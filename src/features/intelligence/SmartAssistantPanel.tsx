...
export function SmartAssistantPanel({ locale, configured, model, options, available, suggestion, loading, error, onOption, onGenerate, onApply, onRefresh, onOpenIssue }: {
  ...
  return (
    <section className="rounded-[var(--qm-panel-radius)] border bg-card p-3.5 shadow-sm">
      ...
      {!configured ? (
        <div className="mt-3 rounded-[var(--qm-control-radius)] border border-dashed bg-muted/10 p-3 text-xs text-muted-foreground">
        ...
        <>
          ...
          <div className="mt-2 grid gap-1.5 sm:grid-cols-2">
            {DATA_KEYS.map((key) => (
              <label key={key} className={`flex items-start gap-2 rounded-[var(--qm-control-radius)] border px-2.5 py-2 text-[11px] ${available[key] ? "cursor-pointer bg-background" : "cursor-not-allowed opacity-45"}`}>
      ...
      {error ? <div className="mt-3 flex items-start gap-2 rounded-[var(--qm-control-radius)] border border-destructive/25 bg-destructive/5 p-2.5 text-[11px] text-destructive"><AlertTriangle className="mt-0.5 size-3.5 shrink-0" /><span>{error}</span></div> : null}
...
function SuggestionPreview({ locale, suggestion, labels: tx, onApply, onOpenIssue }: { locale: AppLocale; suggestion: SmartAssistantSuggestion; labels: ReturnType<typeof labels>; onApply: (value: SmartAssistantSuggestion) => void; onOpenIssue?: (key: string) => void }) {
  ...
  return (
    <div className="mt-3 rounded-[var(--qm-control-radius)] border bg-muted/10 p-3">
