...
export function SmartAssigneeSuggestions({ locale, suggestions, onSelect }: { locale: AppLocale; suggestions: AssigneeSuggestion[]; onSelect: (identity: string) => void }) {
  ...
  return (
    <div className="mt-2 rounded-[var(--qm-control-radius)] border border-dashed bg-muted/10 p-2.5">
      ...
      <div className="flex flex-wrap gap-2">
        {suggestions.map((item) => (
          <button key={item.identity} type="button" onClick={() => onSelect(item.identity)} className="inline-flex min-w-0 items-center gap-2 rounded-[var(--qm-control-radius)] border bg-background px-2.5 py-1.5 text-start text-xs transition-colors hover:border-primary/30 hover:bg-primary/[0.03]">
