...
export function PotentialDuplicatesPanel({ locale, matches, loading, projectChecked, onCheck, onOpen }: {
  ...
  return (
    <div className={cn("mt-2 rounded-[var(--qm-control-radius)] border p-2.5", matches.length ? "border-warning/30 bg-warning/8" : "border-dashed bg-muted/10")}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-1.5 text-[11px] font-medium"><Search className={cn("size-3.5", matches.length ? "text-warning" : "text-muted-foreground")} />{matches.length ? (locale === "fa" ? "تسک های مشابه پیدا شد" : "Potential duplicates") : (projectChecked ? (locale === "fa" ? "مورد مشابه نزدیکی پیدا نشد" : "No close duplicates found") : (locale === "fa" ? "بررسی تکراری بودن" : "Duplicate check"))}</div>
