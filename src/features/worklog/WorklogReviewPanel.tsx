...
import { buildWorklogFallbackComment } from "@/lib/worklog-comment"
...
export function WorklogReviewPanel({ locale, draft, draftMinutes, issues, remainingMinutes, applying, onUpdate, onRemove, onClear, onApply }: {
  ...
  return (
    <Card className="gap-0 border-success/25 py-0 shadow-none ">
      <CardContent className="p-3">
        ...
        {exceedsRemaining ? <div className="mb-3 flex items-start gap-2 rounded-[var(--qm-control-radius)] border border-warning/30 bg-warning/8 px-3 py-2 text-xs text-warning"><AlertTriangle className="mt-0.5 size-4 shrink-0" /><span>{isFa ? `این Draft از زمان باقی مانده امروز بیشتره: ${formatWorklogMinutes(draftMinutes)} در برابر ${formatWorklogMinutes(remainingMinutes)}.` : `This draft exceeds today's remaining target: ${formatWorklogMinutes(draftMinutes)} vs ${formatWorklogMinutes(remainingMinutes)}.`}</span></div> : null}
        <div className="overflow-x-auto rounded-[var(--qm-panel-radius)] border">
          <div className="min-w-[880px]">
            <div className="grid grid-cols-[minmax(300px,1.25fr)_200px_minmax(260px,1fr)_44px] gap-3 border-b bg-muted/20 px-3 py-2 text-[11px] font-semibold uppercase tracking-wide text-muted-foreground"><span>{isFa ? "تسک" : "Issue"}</span><span>{isFa ? "زمان" : "Time"}</span><span>{isFa ? "توضیح Worklog" : "Worklog description"}</span><span /></div>
            {draft.map((entry) => {
              const issue = issueMap.get(entry.issueKey)
              return <div key={entry.issueKey} className="grid grid-cols-[minmax(300px,1.25fr)_200px_minmax(260px,1fr)_44px] items-center gap-3 border-b px-3 py-2.5 last:border-b-0 hover:bg-muted/[0.12]"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-xs text-primary">{entry.issueKey}</span>{issue?.status ? <Badge variant="outline" className="font-normal">{issue.status}</Badge> : null}{issue?.sprintName ? <Badge variant="outline" className="font-normal">{issue.sprintName}</Badge> : null}</div><div className="mt-0.5 truncate text-sm font-medium">{entry.summary}</div></div><div className="flex items-center gap-2"><Input aria-label={isFa ? `دقیقه برای ${entry.issueKey}` : `Minutes for ${entry.issueKey}`} type="number" min={1} max={1440} value={entry.minutes} onChange={(event) => onUpdate(entry.issueKey, { minutes: Math.max(0, Math.round(Number(event.target.value) || 0)) })} className="w-24 tabular-nums" /><span className="whitespace-nowrap rounded-[var(--qm-control-radius)] bg-muted/70 px-2 py-1.5 text-xs font-medium tabular-nums text-muted-foreground">{formatWorklogMinutes(entry.minutes)}</span></div><Input value={entry.comment} onChange={(event) => onUpdate(entry.issueKey, { comment: event.target.value })} placeholder={buildWorklogFallbackComment(entry.issueKey, entry.summary)} /><Button variant="ghost" size="icon-sm" className="text-muted-foreground hover:text-destructive" onClick={() => onRemove(entry.issueKey)} aria-label={isFa ? "حذف" : "Remove"}><Trash2 className="size-4" /></Button></div>
            })}
          </div>
        </div>
