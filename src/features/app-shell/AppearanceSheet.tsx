import { ArrowLeftRight, Grid2X2, Grid3X3, Languages, Palette, Sun } from "lucide-react"
...
export function AppearanceSheet({ open, onOpenChange, locale, t, theme, setTheme, setLocale, accentColor, setAccentColor, reviewLayout, setReviewLayout, gridColumns, setGridColumns, density, setDensity }: {
  open: boolean
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side={locale === "fa" ? "left" : "right"}>
        <SheetHeader><SheetTitle>{t.settings}</SheetTitle><SheetDescription>{t.appTagline}</SheetDescription></SheetHeader>
...
function Segmented({ values, value, onChange }: { values: Array<{ value: string; label: string }>; value: string; onChange: (value: string) => void }) {
  return <div className="grid grid-cols-[repeat(auto-fit,minmax(0,1fr))] gap-1 rounded-lg border bg-muted/25 p-1">{values.map((item) => <Button key={item.value} variant={value === item.value ? "secondary" : "ghost"} size="sm" onClick={() => onChange(item.value)} aria-pressed={value === item.value}>{item.label}</Button>)}</div>
}
