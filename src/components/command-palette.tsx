...
export function CommandPalette({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[2147483646] flex items-start justify-center bg-black/35 px-4 pt-[12vh] backdrop-blur-[2px]" onMouseDown={() => onOpenChange(false)}>
      <div role="dialog" aria-modal="true" aria-label={title} className="w-full max-w-[640px] overflow-hidden rounded-2xl border bg-popover text-popover-foreground shadow-2xl" onMouseDown={(event) => event.stopPropagation()}>
          ...
          {filtered.length ? filtered.map((item, index) => {
            ...
            return (
              <Fragment key={item.id}>
                ...
                <div role="option" aria-selected={index === activeIndex} className={cn("group flex w-full items-center rounded-xl transition-colors", index === activeIndex && "bg-accent text-accent-foreground", item.disabled && "opacity-45")} onMouseEnter={() => !item.disabled && setActiveIndex(index)}>
                  <button type="button" disabled={item.disabled} onClick={() => { if (!item.disabled) { onOpenChange(false); item.onSelect() } }} className={cn("flex min-w-0 flex-1 items-center gap-3 rounded-xl px-3 py-2.5 text-start outline-none", item.disabled && "cursor-not-allowed")}>
                  ...
                  {item.onToggleFavorite ? <button type="button" onClick={(event) => { event.stopPropagation(); item.onToggleFavorite?.() }} className="me-2 grid size-8 shrink-0 place-items-center rounded-lg text-muted-foreground outline-none hover:bg-background/70 hover:text-foreground focus-visible:ring-2 focus-visible:ring-ring" aria-label={item.favorite ? `Remove ${item.label} from favorites` : `Add ${item.label} to favorites`} title={item.favorite ? "Remove favorite" : "Add favorite"}><Star className={cn("size-4", item.favorite && "fill-current text-amber-500")} /></button> : null}
                </div>
              </Fragment>
            )
